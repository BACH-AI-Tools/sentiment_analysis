...
4. 🔍 搜尋或找到本伺服器（`bach-sentiment_analysis`）
5. 🎉 點擊 **「安裝 MCP」** 按鈕
6. ✅ 完成！即可在您的應用中使用
...
這是一個使用 [FastMCP](https://fastmcp.wiki) 自動生成的 MCP 伺服器，用於存取 Sentiment Analysis API。

- **PyPI 套件名**: `bach-sentiment_analysis`
- **版本**: 1.0.0
- **傳輸協定**: stdio
...
```bash
pip install bach-sentiment_analysis
```
...
```bash
# 运行（uvx 会自动安装并运行）
uvx --from bach-sentiment_analysis bach_sentiment_analysis

# 或指定版本
uvx --from bach-sentiment_analysis@latest bach_sentiment_analysis
```
...
**注意**: 請將 `E:\path\to\sentiment_analysis\server.py` 替換為實際的伺服器檔案路徑。
